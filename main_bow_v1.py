...
def target_function(robot, ActionSpace, Vx, Vy, Vz, Wz):
    value = np.array([Vx, Vy, Vz, Wz])
...
def constraint_function(robot, ActionSpace, manager, Vx, Vy, Vz, Wz):
    value = np.array([Vx, Vy, Vz, Wz])
    quad_t = QuadrotorKinematics(initial_position=robot.get_position(), initial_orientation=robot.get_orientation())
    traj = [[x, y, z, psi] for x, y, z, psi in compute_short_traj(quad_t, value)]
...
def main(args):
    ...
    num_trajectories = num_subdivision * 2
    safety_dist = 0.345 * 2
    ...
    while True:
        ...
        print(optimizer.max)
        value = np.array([optimizer.max['params'][x] for x in ["Vx", "Vy", "Vz", "Wz"]])
        quad_t = QuadrotorKinematics(initial_position=robot.get_position(), initial_orientation=robot.get_orientation())
